...
<p align="center"> <img src="assets/image-20250908201526-dqx83s2.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250908202501-uqgau2i.png" width="500"></p>
...
<p align="center"> <img src="assets/image-20250914205645-k4c4zta.png" width="500"></p>​

由于外部信号极有可能会出现脉冲等噪声的影响，为了让信号更加清晰，所以就设置了TTL肖特基触发器（相当于一个滞回比较器）。图为加入TTL肖特基触发器前后对信号处理的效果：<p align="center"> <img src="assets/image-20250910222943-6jvob2f.png" width="450"></p>​
...
<p align="center"> <img src="assets/image-20250909082558-e9iujgb.png" width="500"></p>​

~~（上图中的TTL施密特触发器应为TTL肖特基触发器，下面同理）~~
（其实是都可以用，但因为TTL肖特基触发器因​的TTL电平兼容、滞回抑噪、低成本集成​​等特性，所以GPIO输入模式的信号整形更爱选择这个）   
...
<p align="center"> <img src="assets/image-20250909083108-rddwkc5.png" width="500"></p>​

一般是将外部的模拟信号输入到内部的ADC（模数转换器）中。信号不经过TTL肖特基触发器，不会被整形。

#### 输入上拉

<p align="center"> <img src="assets/image-20250909082904-02ser4u.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250909083034-2kgwzyk.png" width="500"></p>​

就是把电压拉低，拉到GND。与上拉原理相似，图中的下拉电阻处的开关会闭合。

#### 推挽输出

<p align="center"> <img src="assets/image-20250909083703-cdh93ka.png" width="500"></p>​

推挽是指两个三级管或MOS管分别受到互补信号的控制，在一个管子导通的时候另一个总是截止。推挽输出中，PMOS和NMOS都工作，对于高低电平均有较强的驱动能力。

数据寄存器为1，上管导通、下管关断，输出VCC；数据寄存器为0，上管关断，下管导通。

<p align="center"> <img src="assets/image-20250926145247-b4dgxbq.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250909083144-cj7od9i.png" width="500"></p>​

只有NMOS工作，PMOS不工作；

数据寄存器为1，下管断开，输出为高阻模式（漏极相当于开路，所以叫开漏）；数据寄存器为0，下管导通，输出低电平。

<p align="center"> <img src="assets/image-20250926145651-a43b7i8.png" width="450"></p>该模式只有低电平有驱动能力，因此如果想要在使用该模式时还能输出高电平，需要外接上拉电阻（一般为4.7k）。
...
打开CubeMX新建工程后，可以看到右侧显示有芯片及其引脚分布。点击任一想要配置的引脚，在下拉菜单中可以选择GPIO的输入`GPIO_Input`或输出`GPIO_Output`功能。<p align="center"> <img src="assets/image-20250926151410-xybwypj.png"></p>​

点击右侧System Core - GPIO，再点击对应的GPIO编号，可以配置GPIO的各种参数：

<p align="center"> <img src="assets/image-20250909150705-i7logfm.png"width="500"></p>​
...
- ​`Output Open Drain`:开漏输出，只有下拉的NMOS起作用，通常用于某些通信中。
  ...
  <p align="center"> <img src="assets/image-20250909150923-jxz8yt2.png"width="500"></p>
...
以下部分了解即可：
**Maximum output speed (最大输出速度):** 此项用于配置GPIO引脚的输出驱动电路的压摆率（Slew Rate），它直接决定了引脚电平翻转的速度。STM32提供了低速、中速、高速三种模式，选择的原则是在满足应用需求的前提下，尽可能选择较低的速度。

* **为什么不总是用最高速？**
    *  **功耗问题**: 速度越快，驱动管的开关瞬时电流越大，动态功耗也越高。对于功耗敏感的应用，应避免使用不必要的高速模式。
    *  **EMI (电磁干扰) 问题**: 更快的翻转速度意味着信号的上升/下降沿更陡峭，这会产生更强的高频谐波，增加电磁干扰的风险，可能影响到模拟电路或其他敏感信号。

* **速度等级与典型应用:**
    * **Low speed (2MHz):** 适用于对速度要求不高的场合，例如驱动LED、控制继电器、按键扫描等。这是最常规、最省电、EMI影响最小的选择。
    * **Medium speed (10MHz):** 适用于中速通信接口，例如常规速率的SPI通信、I2C等。
    * **High speed (50MHz):** 仅在需要极高翻转速率的场合使用，例如高速SPI、驱动LCD屏，或作为某些高速协议的时钟输出。

**配置建议：** 遵循“**够用即可**”的原则。只有在明确需要高速信号的场景下（如Datasheet要求或协议速率限制），才选择中速或高速模式。在不确定的情况下，**默认使用 `Low` 模式** 是最安全稳妥的做法。   
 #
...
​`HSE`和`HSI`频率在多数情况下是一样的, **[HSI (高速内部时钟) 的频率是固定的（通常为8MHz或16MHz，具体看芯片型号），但其精度和稳定性较差，易受温度和电压影响。HSE (高速外部时钟) 的频率由外部晶振决定，常见值为8MHz、12MHz、25MHz等，可以有多种选择，并且其频率非常稳定。]**，但是外部时钟接的石英晶振比内部的RC震荡电路产生的频率更稳定，所以一般都选`HSE`。
...
<p align="center"> <img src="assets/image-20250926153337-k5nlkzm.png" width="400"></p>​

Cube中`Clock Configuration`界面：

一般来说，在填入正确的外部时钟晶振频率、勾选了`HSE`-`PLLCLK`后，在`HCLK`上输入想要的频率后点击回车键，CubeMX可以自动配置每一外设的频率。

<p align="center"> <img src="assets/image-20250909094643-9ug4yot.png" width="650"></p>​

### TIM介绍

TIM是一个定时器，与时钟树不是一个概念：时钟树控制着整个系统的时钟频率，而TIM是在时钟树分频后的某个频率下的用于计数的外设。简单来说，TIM就是一个计数器。

<p align="center"> <img src="assets/image-20250926155218-d2c0x3b.png" width="500"></p>​

TIM的结构简图

#### TIM的时基单元

<p align="center"> <img src="assets/image-20250920161349-3j2oo63.png" width="500"></p>​

##### 时钟来源

<p align="center"> <img src="assets/image-20250926154309-vh5rb68.png" width="500"></p>​
...
- 计数方式：
  ...
  - TIM有三种计数方式：
    ...
    - 中心对齐（一般不用）<p align="center"> <img src="assets/image-20250926154740-yzo3os3.png" width="500"></p>​

##### 重复计数器RCR：

<p align="center"> <img src="assets/image-20250926154904-h16ohep.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250920163057-dpaoz6a.png" width="500"></p>​

假如外部时钟信号为32.768kHz，每当有一个update事件就表示响铃1次；秒针每秒转1次，也就是说，PSC降频后的信号为1Hz，同时计数方式为上计数。所以

- 将PSC设置为32768-1，得到1Hz
- 将ARR设置为60-1，使得计数器每计60个数（即60秒）就溢出一次，产生更新事件，代表秒针转了1圈；
...
<p align="center"> <img src="assets/image-20250926160600-ywjd01u.png" width="400"></p>  
比方说我们想向寄存器写值。值先进入影子寄存器，在某一事件发生后，影子寄存器里的值进入到活动寄存器，值才生效。

这是为了安全起见，比如在定时器运行的过程中突然改变ARR的值：

<p align="center"> <img src="assets/image-20250926160621-ky4vdx8.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250926160836-58ckbvt.png" width="500"></p>​
...
定时器的中断中，更新事件可以作为一个中断源来产生中断，因此我们利用中断函数来给更新事件计数。<p align="center"> <img src="assets/image-20250921095932-ji3kobp.png" width="500"></p>​

记得给update interrupt使能：

<p align="center"> <img src="assets/image-20250921100012-xxdokvf.png" width="500"></p>​

##### 以中断方式使用时基单元的函数：

<p align="center"> <img src="assets/image-20250921100153-t26966h.png" width="550"></p>​
...
CCR： `Capture / Compare Register` 捕获比较寄存器<p align="center"> <img src="assets/image-20250926164140-uhstp9h.png" width="500"></p>  
可以调制脉冲的周期 or 占空比，从而改变高&低电平的宽度，或等效模拟电压的大小（因应用需求而异）。

<p align="center"> <img src="assets/image-20250921201755-j8kagvp.png"width="500"></p>​
...
<p align="center"> <img src="assets/image-20250909100733-3jzf2g6.png" width="700"></p>​

##### 支持实现输出比较功能的内部结构：

<p align="center"> <img src="assets/image-20250921202053-kmf5psv.png" width="500"></p>​

###### 模式选择：

根据CCR与CNT的比较结果，有不同的输出方式。一共有8种，一般用最后两种。

<p align="center"> <img src="assets/image-20250921202222-10lib03.png" width="500"></p>​

###### 互补输出：

只有高级定时器才有互补输出。

如在使用PWM驱动MOS管时使用：<p align="center"> <img src="assets/image-20250921202341-759bwnn.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250921202453-7bohgx7.png"width="500"></p>​

- 一路选正常，一路选互补：

<p align="center"> <img src="assets/image-20250921202441-eq8m3jr.png"width="500"></p>​
...
- **串行通信：** 数据位通过单根线一一发送，下图显示了二进制（0100 0011）中字母“C”的串行传输。数据在时钟信号的特定 **跳变沿（上升沿或下降沿）** 被采样。高电平为1，低电平为0。<p align="center"> <img src="assets/image-20250910134549-s1yq4vf.png"></p>​

- **并行通信：** 数据位在导线中同时传输。下图显示了二进制（0100 0011）中字母“C”的并行传输。当时钟线为高电平时，8个数据位同时接受数据。高电平为1，低电平为0。

<p align="center"> <img src="assets/image-20250910134723-w4towij.png"width="500"></p>​
...
(USART：通用同步/异步收发器，`Universal Synchronous Asynchronous Receiver/Transmitter`）<p align="center"> <img src="assets/image-20250910131952-ih0737c.png" width="450"></p>​
...
- 点对点：A只能与B通信
- 内部使用推挽输出   
  （P.S. 由于UART是点对点通信，不存在总线冲突的可能，因此使用推挽输出可以提供更强的驱动能力和更快的边沿速率。）   
 
   

  <p align="center"> <img src="assets/image-20250910165351-eslxz5f.png"width="410"></p>​
...
<p align="center"> <img src="assets/image-20250910132825-0zcb9ix.png" width="610"></p>​
...
- 紧接着起始位之后。数据位的个数可以是4、5、6、7、8等，依照2进制构成一个字符。通常采用ASCII码
  ...
  - ASCII码：`American Standard Code for Information Interchange`，即美国信息交换标准代码，主要用于显示现代英语。每一组二进制数代表特定字符<p align="center"> <img src="assets/image-20250910133543-bzfe0nu.png" width="600"></p>​
- 从最低位开始传送，靠时钟定位。

- #### 奇偶校验位

- 奇校验：统计数据包里出现多少个1，假如出现奇数次1则该校验位置‘0’，假如出现偶数次1则该校验位置‘1’，以此来保证整段数据中1的个数为奇数次，借此校验资料传送的正确性。
- 偶校验：同理反之。
- 需要在两端设备同时都设置
...
- #### 波特率
<p align="center"> <img src="assets/image-20250915143642-84xfsw0.png" width="510"></p>​
...
<p align="center"> <img src="assets/image-20250910135118-msil687.png"width="500"></p>​

电平使用负逻辑，逻辑1定义为-3V - -15V，逻辑0定义为+3V - +15V

<p align="center"> <img src="assets/image-20250910140106-ebkpzo5.png"width="500"></p>​

使用相应的芯片就可以将TTL电平变为232电平

<p align="center"> <img src="assets/image-20250910135355-4ou1u2k.png"width="500"></p>​
...
<p align="center"> <img src="assets/image-20250910135710-tmx2q80.png"width="250"></p>​


逻辑1：A相对B电压在+2 - +6V间； 逻辑0：A相对B电压在-2 - -6V间

<p align="center"> <img src="assets/image-20250910135542-1wj9so6.png"width="400"></p>​

‍

<p align="center"> <img src="assets/image-20250910135621-9r2odju.png"width="400"></p>​

传输距离更长，频率更高：

<p align="center"> <img src="assets/image-20250910135833-yd23m0d.png"width="400"></p>​

采用半双工，可进行一主多从的组网通讯，即和很多设备进行通讯。

<p align="center"> <img src="assets/image-20250910135939-e4ucudv.png"width="500"></p>​
...
<p align="center"> <img src="assets/image-20250910150544-h3cwebr.png"width="500"></p>​

- SS：决定主机与哪一从机通信，低电平才有效。不同的从机有不同的SS线（因此SPI内部结构可以是推挽输出）  
   **SS (Slave Select) 更常见的叫法是 CS (Chip Select) 或 NSS (Negative Slave Select)**
- GND：图中没有，但是主从机的地线一定是接在一起的
- SCK：时钟线，信号由主机产生
- MOSI：`Master Output, Slave Input`，主机输出，从机接收（相当于数据线）<p align="center"> <img src="assets/image-20250910153348-u4s02ff.png" width="500"></p>​
- MISO：`Master Input, Slave Output`，主机接收，从机输出<p align="center"> <img src="assets/image-20250910153336-amr5kmp.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250926171006-3ocutzh.png"width="500"></p>​
...
<p align="center"> <img src="assets/image-20250910150644-c0slyez.png" width="600"></p>​
...
<p align="center"> <img src="assets/image-20250910141729-13mpbt4.png"width="500"></p>​
...
- 任意引脚写0，该引脚输出低电压，总线电压被拉低<p align="center"> <img src="assets/image-20250916134240-21j6j2y.png" width="500"></p>
...
<p align="center"> <img src="assets/image-20250926173758-31hunz6.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250926173835-hb4dt1t.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250926173852-z0xhv5f.png" width="500"></p>​

#### 停止位

在SCL为高电平时，向SDA发送上升沿。

<p align="center"> <img src="assets/image-20250926173905-7kntqcm.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250926173954-iaogors.png" width="500"></p>​

### 通信速率：

了解即可。

<p align="center"> <img src="assets/image-20250916135136-559p832.png" width="400"></p>​
...
一般情况需要使用CAN收发芯片，并且在需要走特性阻抗为120欧（typical情形）的差分线（长度需要相同），并且需要使用2个120欧的终端电阻跨接在CAN_H和CAN_L的两个终端。<p align="center"> <img src="assets/image-20250910163204-9wqe6ol.png" width="500"></p>​
...
<p align="center"> <img src="assets/image-20250910171119-wu5nnao.png" width="400"></p>​
...
<p align="center"> <img src="assets/image-20250910152256-cv98nfq.png" width="500"></p>​
